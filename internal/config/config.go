...
type Config struct {
	...
	DeleteFilesFromFilesystem bool         `yaml:"delete_files_from_filesystem"` // Delete files from filesystem when deleting from DB (DANGEROUS!)
...
func Default() *Config {
	return &Config{
		...
		DeleteFilesFromFilesystem: false, // Default to safe behavior (DB only)
		DBMaxOpenConns:           25,
		DBMaxIdleConns:       5,
